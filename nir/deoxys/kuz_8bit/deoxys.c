                         ...
                         uint8_t *ciphertext, size_t *c_len)
{
    ...
    if(ass_data_len) {
        ...
        while (16*(i+1) <= ass_data_len) {
            ...
            kuz_encrypt_block(ass_data+16*i, temp);
            /* Update Auth value */
        ...
        if ( ass_data_len > 16*i ) {
            ...
            kuz_encrypt_block(last_block, temp);
    ...
    while (16*(i+1) <= m_len) {
        ...
        kuz_encrypt_block(message+16*i, ciphertext+16*i);
        i++;
    }
    ...
    if (m_len > 16*i) {
        ...
        kuz_encrypt_block(zero_block, Pad);
        ...
        kuz_encrypt_block(Checksum, Final);
    }
    else{
        //set_block_number_in_tweak(tweak, i);
                       ...
                       const uint8_t *ciphertext, size_t c_len)
{
    ...
    while(16*(i+1)<=c_len) {
        ...
        kuz_decrypt_block(message+16*i, ciphertext+16*i);
        xor_values(Checksum, message+16*i);
        i++;
    }
